  ...
  <div class="dashboard-container">
    <div class="dashboard-header">
      ...
      <div class="header-actions">
        ...
        <button @click="loadDashboardData" class="refresh-button text-bg-light" :disabled="isLoading" title="Refresh Data">
      ...
      <div class="list-card overdue-list">
        ...
        <ul v-if="lists.overdueRentals.length > 0">
          <li v-for="rental in lists.overdueRentals" :key="rental.uuid" @click="viewRental(rental.uuid)" class="clickable">
            ...
            <span class="due-date">Due: {{ formatDate(rental.expectedReturnDate) }}</span>
          </li>
        </ul>
      ...
      <div class="list-card collections-list">
        ...
        <ul v-if="lists.collectionsDueToday.length > 0">
          <li v-for="booking in lists.collectionsDueToday" :key="booking.uuid" @click="viewBooking(booking.uuid)" class="clickable">
            ...
            <span class="due-date collections">From: {{ formatTime(booking.bookingStartDate) }}</span>
          </li>
        </ul>
        <p v-else class="no-items-message">
...
export default {
  ...
  methods: {
    /**
     * Fetches all data for the dashboard from the dedicated API service.
     * Handles loading states, error states, and ensures component is
     * still mounted before updating data.
     */
    async loadDashboardData() {
      ...
      try {
        ...
        if (this._isMounted) {
          this.error = err.message || "Failed to load dashboard data. Please check the console for details.";
    ...
    /**
     * Navigates to the details page for a specific rental.
     * @param {string} rentalUuid - The UUID of the rental to view.
     */
    ...
    /**
     * Navigates to the details page for a specific booking.
     * @param {string} bookingUuid - The UUID of the booking to view.
     */
    viewBooking(bookingUuid) {
      this.$router.push({ name: 'ViewBooking', params: { uuid: bookingUuid } });
    },
...
.kpi-router-link {
  ...
  border-radius: 12px; /* Apply border-radius to link for hover effect */
}
...
.chart-card h3 { margin-top: 0; text-align: center; }
...
@media (max-width: 1200px) {
  ...
  .kpi-card-container { grid-template-columns: repeat(2, 1fr); }
}

@media (max-width: 768px) {
  .dashboard-grid, .kpi-card-container { grid-template-columns: 1fr; }
